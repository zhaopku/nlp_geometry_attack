...
class MyDataSet(Dataset):
	def __init__(self, samples, max_steps):
	...
	def __getitem__(self, index):
		...
		return sample.id, torch.tensor(word_ids), length, sample.label, torch.tensor(stopwords_mask), torch.tensor(mask)
...
class IMDBData:
	...
	def _create_samples(self, path):
		...
		for tag in tags:
			file_names = os.listdir(os.path.join(path, tag))
			for file_name in tqdm(file_names, desc='iterating {}/{}'.format(path, tag)):
				with open(os.path.join(path, tag, file_name), 'r') as file:
	...
	def _create_data(self):
		...
		train_path = os.path.join(self.args.data_dir, self.args.dataset, self.args.train_path)
		test_path = os.path.join(self.args.data_dir, self.args.dataset, self.args.test_path)
	...
	def _read_sents(self, path):
		...
		all_words = []
		for file_name in tqdm(file_names, desc='iterating over {}'.format(path)):
	...
	def _build_vocab(self, train_path, test_path):
		...
		all_train_words_pos = self._read_sents(os.path.join(train_path, 'pos'))
		all_train_words_neg = self._read_sents(os.path.join(train_path, 'neg'))
		all_train_words = all_train_words_pos + all_train_words_neg

		all_test_words_pos = self._read_sents(os.path.join(test_path, 'pos'))
		all_test_words_neg = self._read_sents(os.path.join(test_path, 'neg'))
		all_test_words = all_test_words_pos + all_test_words_neg
